      ...
      <div className="w-full mx-auto mb-10 text-left md:w-3/4 lg:w-1/2">
        <div className="pb-6 mb-6 border-b border-gray-200">
          ...
          <p className="text-gray-500 italics list-none text-sm">
              {categories.map(category => <li key={category}>CATEGORY &rarr; {category}</li>)}
            </p>
        </div>
        ...
        <div className="">
          {mainImage && (
            <div>
              <img src={urlFor(mainImage).width(1200).url()} />
            </div>
          )}
        </div>
      </div>
...
{/* ads pagination buttons to the bottom of the blog post; dunno how to wire it up! */}
    <div className="hidden flex container mx-auto justify-center">
    <button className="border border-indigo-500 text-indigo-500 rounded-sm font-bold py-4 px-6 mr-2 flex items-center hover:bg-indigo-500 hover:text-white">
        &larr; Previous post
    </button>
    <button className="border border-indigo-500 bg-indigo-500 text-white rounded-sm font-bold py-4 px-6 ml-2 flex items-center">
        Next post &rarr;
    </button>
    </div>
  </>
